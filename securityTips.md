...
3. Now we will add our validator ID and its new IP to the `config.toml` We will add it in 2 different places. ID@IP:PORT in the section _“persisten_peers”_ and the ID _“private_peers_id”_ on **Server-A**. This will connect both, sentry and validator next time we reload the daemon.
